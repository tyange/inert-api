...
- `GET /u/:username/profile` — 유저 공개 프로필 (username, display_name, avatar_url, bio)
- `PUT /auth/me` — 프로필 수정 (인증 필요)
...
## DB 스키마 핵심
- `users`: user_id, email, username, google_sub, display_name, avatar_url, bio (100자 제한)
...
- 완료: 유저 공개 프로필 엔드포인트 (bio 포함)
- 완료: bio 100자 제한 validation
